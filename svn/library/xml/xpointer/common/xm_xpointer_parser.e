...
feature {NONE} -- Implementation
				...
				if a_tokenizer.is_lexical_error then
					...
					if not a_tokenizer.is_lexical_error then
						if a_tokenizer.last_token /= String_token then
							...
							l_current_scheme_name := a_tokenizer.last_token_value
							current_scheme_name := l_current_scheme_name
							if not is_qname (l_current_scheme_name) then
								set_error ("XPointer scheme name is not a lexical QName: " + l_current_scheme_name, "XPOINTER_LEXICAL")
							else
								a_tokenizer.next
							end
						end
	...
	store_current_scheme is
		...
		local
			...
			l_data: like current_scheme_data
		do
			l_name := current_scheme_name
			check l_name /= Void end -- implied by `lexical_qname'
			scheme_sequence.force_last (l_name)
			l_data := current_scheme_data
			check l_data /= Void end -- implied by `good_data'
			scheme_data.force_last (l_data)
		end
